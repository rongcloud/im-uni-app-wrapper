import {
  isArray, isFunction, isNumber, isNumberArray, isObject, isString, isUndefined, notEmptyArray, notEmptyString,
} from './validator';
...
const validators = {
  ...
  [AssertRules.BOOLEAN]: (value: any) => typeof value === 'boolean',
...
export const assert = (
  key: string,
  value: any,
  validator: AssertRules | ((value: any) => boolean),
...
export const validate = (
  ...
  let validatorFunc = validators[validator as AssertRules] || validator;
  const isValid = required ? validatorFunc(value) : (isUndefined(value) || value == null || validatorFunc(value));
  if (!isValid) {
    // 打印无效参数到控制台便于定位问题
    // eslint-disable-next-line no-console
    console?.error(`'${key}' is invalid: ${JSON.stringify(value)}, the supported ${RulesDesc[validator as AssertRules]}`);
  }
  return isValid;
};
